...
def calculate_demographic_data(print_data=True):
    ...
    race_count = df.groupby("race", sort=False)["race"].count()
    ...
    rich_percentage = round((num_min_workers["salary"][num_min_workers["salary"] == ">50K"].count() / num_min_workers["salary"].count()) * 100,1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = (df["native-country"][df["salary"] == ">50K"].value_counts() / df["native-country"].value_counts()).idxmax()
    highest_earning_country_percentage = round((df["native-country"][df["native-country"] == highest_earning_country][df["salary"] == ">50K"].count() / df["native-country"][df["native-country"] == highest_earning_country].count()) * 100,1)
